...
@Composable
fun CameraBodyListScreen(onAdd: () -> Unit, onEdit: (String) -> Unit) {
    val container = appContainer()
    val viewModel: CameraBodyListViewModel = viewModel(
        factory = ExposuresViewModelFactory(container.repository, container.syncPusher, container.dataLayerClient),
    )
    val state by viewModel.uiState.collectAsState()

    Scaffold(
        topBar = { TopAppBar(title = { Text("Camera Bodies") }) },
        floatingActionButton = { FloatingActionButton(onClick = onAdd) { Text("+", modifier = Modifier.padding(8.dp)) } },
    ) { padding ->
        LazyColumn(modifier = Modifier.fillMaxWidth().padding(padding)) {
            items(state.cameraBodies) { body: CameraBody ->
                ListItem(
                    headlineContent = { Text(body.name) },
                    supportingContent = { Text(body.manufacturer) },
                    modifier = Modifier.fillMaxWidth().clickable { onEdit(body.id) },
                )
            }
        }
    }
